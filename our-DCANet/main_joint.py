...
def run_train(train_data_file, dev_data_file):
    ...
    vocab_list = [word.strip() for word in vocab_file] # 单词表
    ...
    embedding_word, vocab = process_emb(embeddings, emb_dim=config.emb_dim) # embbedinng_word每一行是一个词的300dim-embedding，vocab是一个word2id的字典

    idx2intent, intent2idx = lord_label_dict(config.data_path + "intent_label.txt") # 两个字典: intent_label 与 index 的转换
    idx2slot, slot2idx = lord_label_dict(config.data_path + "slot_label.txt") # 两个字典: slot_BIO 与 index 的转换
