...
class CRF_LSTM_BERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.rnn = getattr(nn, self.cell)(BERT_VOCAO_SIZE, config.hidden_size // 2, num_layers=config.num_layer, bidirectional=True, batch_first=True).to(self.device)
        self.dropout_layer = nn.Dropout(p=config.dropout).to(self.device)
        self.hidden_to_tag_layer = nn.Linear(config.hidden_size, config.num_tags).to(self.device)   # 这里将隐藏层的输出映射到标签的数量
        self.crf = CRF(config.num_tags, batch_first=True).to(self.device)
