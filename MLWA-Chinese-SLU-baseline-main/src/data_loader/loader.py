...
class DatasetManager(object):
    ...
    def __read_file(file_path, max_char_seq_len):
        ...
        pre_tag, word = "S", ""
        def segment_and_label(sentence):
            words = pseg.cut(sentence)
            labeled_tokens = []

            for word, flag in words:
                if len(word) == 1:
                    labeled_tokens.append(f"B")
                else:
                    labeled_tokens.append(f"B")
                    for i in range(1, len(word)-1):
                        labeled_tokens.append(f"I")
                    labeled_tokens.append(f"E")

            return labeled_tokens

        def tackle_space(my_str):
            res = my_str.replace(" ", "#")
            return res
        def convert_dataset(original_dataset):
            ret = ""
            for entry in original_dataset:
                manual_transcript = entry["manual_transcript"]
                asr_1best = entry["asr_1best"]
                l1 = len(asr_1best)
                l2 = len(manual_transcript)
                if (l1 > l2):
                    manual_transcript = manual_transcript + "#" * (l1 - l2)
                elif (l1 < l2):
                    asr_1best = asr_1best + "#" * (l2 - l1)
                manual_transcript = tackle_space(manual_transcript)
                asr_1best = tackle_space(asr_1best)
                semantic = entry["semantic"]
                converted_tokens = []
                if(len(semantic) == 0):
                    converted_tokens.extend(["O"] * len(manual_transcript))   
                    for i in range(len(manual_transcript)):
                        ret += (asr_1best[i] + ' ' + converted_tokens[i] + ' ' + segment_and_label(asr_1best)[i] + "\n")
                    ret += ("none" + "\n" + "\n")
                    continue
                start_idx = 0
                end_idx = 0
                ...
                for sem in semantic:
                    if(sem[0] == "deny"):
                        flag = False
                    start_idx = manual_transcript.find(sem[2])
                    if (start_idx == -1): 
                        continue
                    converted_tokens.extend(["O"] * (start_idx - end_idx))
                    end_idx = start_idx + len(sem[2])
                    if (len(sem[2]) >= 2):
                        converted_tokens.extend(["B-" + sem[1]] + ["I-" + sem[1]] * (len(sem[2]) - 2) + ["E-" + sem[1]])
                    else:
                        converted_tokens.extend(["B-" + sem[1]])
                converted_tokens.extend(["O"] * (len(manual_transcript) - end_idx))
                for i in range(len(manual_transcript)):
                    ret += (asr_1best[i] + ' ' + converted_tokens[i] + ' ' + segment_and_label(asr_1best)[i] + "\n")
                if (flag):
                    ret += ("inform" + "\n" + "\n")
            ...
            for line in list_line:
                ...
                if len(items) == 1:
                    if len(word) != 0:
                        ...
                        #print(word, align_info, char_text)
                    try:
