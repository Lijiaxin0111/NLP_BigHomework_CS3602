...
class Processor(object):
    ...
    def train(self):
        ...
        for epoch in range(0, self.__dataset.num_epoch):
            ...
            plt.show()"""
            time_start = time.time()
            dev_f1_score, dev_slot_acc, dev_acc, dev_sent_acc = self.estimate(if_dev=True, test_batch=self.__batch_size * 2)

            # dev_metric = dev_f1_score
            dev_metric = dev_sent_acc if not self.__dataset.golden_intent else dev_f1_score

            if dev_metric > best_dev_metric:
                test_f1, slot_acc, test_acc, test_sent_acc = self.estimate(if_dev=False, test_batch=self.__batch_size * 2)
                ...
                print('\nTest result: slot f1 score: {:.6f}, slot acc score: {:.6f}, intent acc score: {:.6f}, '
                      'semantic accuracy score: {:.6f}.'.format(test_f1, slot_acc, test_acc, test_sent_acc))
                ...
                print('[Epoch {:2d}]: In validation process, the slot f1 score is {:2.6f}, '
                      'the slot acc is {:2.6f}, the intent acc is {:2.6f}, the semantic acc is {:2.6f}, '
                      'cost about {:2.6f} seconds.\n'.format(epoch, dev_f1_score, dev_slot_acc, dev_acc, dev_sent_acc, time_con))
    ...
    def estimate(self, if_dev, test_batch=100):
        ...
        slot_acc = Evaluator.slot_accuracy(pred_slot, real_slot)
        ...
        return slot_f1, slot_acc, intent_acc, sent_acc
